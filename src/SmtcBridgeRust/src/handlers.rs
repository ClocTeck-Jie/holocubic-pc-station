        ...
        Ok(mut status) => {
            ...
            if status.connected {
                ...
                let (found, meta) = if source_name == "qqmusic" {
                    let (f, meta) = state.qqmusic.resolve(&mut status).await;
                    if f.lines.is_empty() {
                ...
                if status.album.is_empty() && !meta.album.is_empty() {
                    status.album.clone_from(&meta.album);
                }
                ...
                if !meta.cover_url.is_empty()
                    && !status.cover_provider.is_empty()
                    && !status.cover_id_text.is_empty()
                {
                    status.cover_url.clone_from(&meta.cover_url);
                } else {
...
pub async fn handle_cover(State(state): State<Arc<AppState>>, Query(params): Query<CoverQuery>) -> Response {
    let result: Result<Response, (StatusCode, String)> = async {
        ...
        if provider.is_empty() {
            provider = if params.ncm_id.is_some() { "netease" } else { "smtc" }.to_string();
        }
        ...
        if provider == "smtc" {
            ...
            let now = Instant::now();
            {
                ...
                if let Some((at, cached_size, ref body, ref ct)) = *cache {
                    if cached_size == size && now.duration_since(at).as_millis() < THUMBNAIL_CACHE_MS as u128 {
            ...
            *cache = Some((now, size, resized.clone(), "image/jpeg".to_string()));
        ...
        if canonical == "qqmusic" {
            let size = params.size.unwrap_or(COVER_SIZE_DEFAULT).clamp(COVER_SIZE_MIN, COVER_SIZE_MAX);
            let candidates = state.qqmusic.cover_candidates(&id_text, &provider);
            let mut last_error = "cover not found".to_string();
            for url in candidates {
                match fetch_cover_buffer(&state.http_client, &url, "https://y.qq.com/").await {
                    Ok((body, _)) => match resize_cover_jpeg(&body, size) {
                        Ok(resized) => return Ok(binary_response(resized, "image/jpeg", true)),
                        Err(e) => last_error = e,
                    },
                    Err(e) => last_error = e,
                }
            }
            return Err((StatusCode::NOT_FOUND, last_error));
        }
