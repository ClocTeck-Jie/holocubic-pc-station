...
pub fn resize_cover_jpeg(buffer: &[u8], size: u32) -> Result<Vec<u8>, String> {
    ...
    // `scale` fills the square, so one axis is commonly larger than `size`.
    // Keep the negative offset: clamping it to zero crops from the top-left.
    let x = (size as i64 - new_w as i64) / 2;
    let y = (size as i64 - new_h as i64) / 2;
