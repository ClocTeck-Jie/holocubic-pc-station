...
/// Difference between the Windows 1601 epoch and Unix 1970 epoch in 100 ns ticks.
const WINDOWS_TO_UNIX_EPOCH_TICKS: i64 = 116_444_736_000_000_000;

fn windows_datetime_to_unix_ms(value: windows::Foundation::DateTime) -> i64 {
    (value.UniversalTime - WINDOWS_TO_UNIX_EPOCH_TICKS) / 10_000
}

fn effective_position_ms(
    raw_position_ms: i64,
    duration_ms: i64,
    playback_status: &str,
    ...
    let mut position = raw_position_ms.max(0);
    if playback_status.eq_ignore_ascii_case("playing") && timeline_updated_at_ms > 0 {
        // Some players (notably QQ Music) expose Position as a snapshot and only
        // update it on seek/track/state changes. LastUpdatedTime identifies when
        // that snapshot was captured, so advance it with the PC clock.
        let elapsed = now_ms.saturating_sub(timeline_updated_at_ms);
        if (0..=24 * 60 * 60 * 1000).contains(&elapsed) {
...
#[cfg(test)]
mod tests {
    use super::effective_position_ms;

    #[test]
    fn playing_position_advances_from_timeline_timestamp() {
        assert_eq!(effective_position_ms(10_000, 90_000, "Playing", 1_000_000, 1_002_500), 12_500);
    }

    #[test]
    fn paused_position_does_not_advance() {
        assert_eq!(effective_position_ms(10_000, 90_000, "Paused", 1_000_000, 1_002_500), 10_000);
    }

    #[test]
    fn extrapolated_position_is_clamped_to_duration() {
        assert_eq!(effective_position_ms(89_000, 90_000, "Playing", 1_000_000, 1_010_000), 90_000);
    }
}
        ...
        tokio::task::spawn_blocking(|| {
            ...
            for candidate in sessions.into_iter() {
                ...
                if score > best_score {
                    ...
                    let raw_position_ms = (pos_ticks / 10000).max(0);
                    let timeline_updated_at_ms = timeline.as_ref()
                        .and_then(|tl| tl.LastUpdatedTime().ok())
                        .map(windows_datetime_to_unix_ms)
                        .unwrap_or(0);
                    let position_ms = effective_position_ms(
                        raw_position_ms,
                        duration_ms,
                        &playback_status,
