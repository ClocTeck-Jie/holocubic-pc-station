...
internal sealed class AsusSensorReader : IVendorSensorProvider
{
    ...
    public string ProviderId => "asus-atkacpi";
    ...
    public VendorSensorSnapshot Read()
    {
        if (!IsAvailable) return new(false, null, null, null);
        lock (_sync)
        {
