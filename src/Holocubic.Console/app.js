...
function refresh({settings=false,quiet=false}={}){
 ...
 request.promise=(async()=>{try{
  const result=await api('/api/system/state','GET',undefined,address);if(!current())return;if(!result||!Array.isArray(result.apps))throw Error('设备返回数据不正确');state.system=result;state.online=true;loadAppIcons().catch(()=>{});
  try{const services=await api('/api/system/services','GET',undefined,address);if(!current())return;state.services=services.services||[];}catch(e){if(!current())return;state.services=[];}
  if(request.settings||!state.settingsLoaded){try{const value=await api('/api/system/fs/file?path='+encodeURIComponent('/sd/apps/settings.json'),'GET',undefined,address);if(!current())return;if(!value||typeof value!=='object'||Array.isArray(value))throw Error('设置文件格式错误');state.settings=value;state.settingsAddress=address;state.settingsLoaded=true;if((state.settingsEditVersion||0)===request.editVersion)state.dirty=false;}catch(e){if(!current())return;state.settingsLoaded=false;if(!quiet)toast('读取设备设置失败：'+e.message);}}
 }catch(e){if(current()){state.online=false;if(!quiet)toast('连接失败：'+e.message);}}
 finally{const active=current();if(state.refreshRequest===request){state.refreshRequest=null;state.busy=false;}if(active){syncChrome();if(state.page==='home'&&!state.dirty)renderHome();}}
...
async function saveSettings(){
 ...
 await api('/api/system/fs/upload?path='+encodeURIComponent('/sd/apps/settings.json'),'PUT',next,address);
 try{await api('/display/api/settings?brightness='+next.brightness,'POST',undefined,address);if(address===state.prefs.active)toast('设置已保存到设备');}catch(e){if(address===state.prefs.active)toast('设置文件已保存，亮度应用失败：'+e.message);}
...
document.addEventListener('click',async event=>{
 ...
 try{
  ...
  else if(target.dataset.device){state.prefs=await call('selectDevice',{ip:target.dataset.device});state.system=null;state.appIcons={};state.iconKey='';state.settingsLoaded=false;state.dirty=false;home()}
  else if(target.dataset.found){state.prefs=await call('saveDevice',{ip:target.dataset.found,verify:true});await call('selectDevice',{ip:state.prefs.active});state.system=null;state.appIcons={};state.iconKey='';state.settingsLoaded=false;state.dirty=false;home()}
  ...
  else if(target.dataset.removeDevice){state.prefs=await call('removeDevice',{ip:target.dataset.removeDevice});await call('selectDevice',{ip:state.prefs.active});state.system=null;state.appIcons={};state.iconKey='';state.settingsLoaded=false;state.dirty=false;home();await openDrawer('settings')}
...
document.addEventListener('submit',async event=>{event.preventDefault();const button=event.target.querySelector('button[type=submit],button.btn.primary');if(button)button.disabled=true;try{
 ...
 if(event.target.id==='addDeviceForm'){state.prefs=await call('saveDevice',{ip:$('#newDeviceIp').value,name:$('#newDeviceName').value,verify:!$('#saveOffline').checked});await call('selectDevice',{ip:state.prefs.active});state.system=null;state.appIcons={};state.iconKey='';state.settingsLoaded=false;state.dirty=false;home();toast('设备已添加')}
...
document.addEventListener('input',e=>{if(e.target.closest('#deviceSettingsForm')){state.settingsEditVersion=(state.settingsEditVersion||0)+1;state.dirty=true;$('#dirtyNote').textContent='设置已修改，等待保存';if(e.target.id==='brightness')$('#brightnessText').textContent=e.target.value+'%'}});
...
fillIcons();
call('init').then(async result=>{state.prefs=result.prefs;HoloI18n.setLanguage(result.prefs.uiLanguage||'zh-CN');state.version=result.version;$('#versionBadge').textContent='v'+state.version;syncChrome();renderHome();await refresh({settings:true});setInterval(()=>{if(state.page==='home'&&!state.drawer&&!state.modal&&!state.dirty&&document.visibilityState==='visible'&&!document.activeElement?.matches('input,select'))refresh({quiet:true})},12000)}).catch(e=>{$('#main').innerHTML=header('初始化失败')+`<p>${esc(e.message)}</p>`});
